...
!define RUN_VALUE_NAME "ZhuQianMo"
...
Function DetectExistingInstallDir
  ...
  ${AndIf} ${FileExists} "$0\${APP_EXE}"
    ...
    Return
  ${EndIf}
  ${If} ${FileExists} "${OLD_INSTALL_DIR}\${APP_EXE}"
    StrCpy $ExistingInstallDir "${OLD_INSTALL_DIR}"
  ${EndIf}
FunctionEnd
...
Section "Install"
  ...
  SetOutPath "$INSTDIR"
  File "${PAYLOAD_DIR}/CloudRelayPublisher.exe"
  File /nonfatal "${PAYLOAD_DIR}/WebView2Loader.dll"
  File "${PAYLOAD_DIR}/README.txt"
  SetOverwrite off
  File "/oname=desktop-config.json" "${PAYLOAD_DIR}/desktop-config.json"
  SetOverwrite on
