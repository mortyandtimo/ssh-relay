...
if [ -z "${CERT_KEEPER_CERT_SYNC_SECRET:-}" ]; then
    CERT_KEEPER_CERT_SYNC_SECRET=$(head -c 24 /dev/urandom | base64 | tr -d '/+=' | head -c 32)
fi
...
CERT_KEEPER_CERT_SYNC_SECRET=${CERT_KEEPER_CERT_SYNC_SECRET}
...
elif ! grep -q '^CERT_KEEPER_CERT_SYNC_SECRET=' "$ENV_FILE"; then
    printf '\nCERT_KEEPER_CERT_SYNC_SECRET=%s\n' "$CERT_KEEPER_CERT_SYNC_SECRET" >>"$ENV_FILE"
    chmod 600 "$ENV_FILE"
    echo "Added CERT_KEEPER_CERT_SYNC_SECRET to $ENV_FILE"
fi
